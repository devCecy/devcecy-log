...
`알고리즘`하면 '알고리즘 문제를 푼다'라는 문장이 같이 떠오르는데요, `알고리즘`은 문제 자체를 나타내기보다는 문제를 해결하기위해 사용하는 일련의 단계 혹은 절차를 뜻하는 것입니다. 이번 글에서는 문제를 해결하 위한 가장 기본적인 알고리즘인 `탐색 알고리즘`, `정렬 알고리즘`, `경로 탐색 알고리즘`, `군집화 알고리즘`에 대해 알아 보도록 하겠습니다.
